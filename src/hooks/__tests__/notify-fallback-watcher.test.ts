...
async function waitFor(predicate: () => Promise<boolean>, timeoutMs: number = 10000, stepMs: number = 50): Promise<void> {
...
const WATCHER_PROCESS_EXIT_TIMEOUT_MS = 15_000;
const WATCHER_PROCESS_OBSERVATION_TIMEOUT_MS = 15_000;

async function waitForExit(child: ReturnType<typeof spawn>, timeoutMs: number = WATCHER_PROCESS_EXIT_TIMEOUT_MS): Promise<void> {
...
`
      ...
      }, WATCHER_PROCESS_OBSERVATION_TIMEOUT_MS, 75);
...
`;
      ...
      await writeFile(join(wd, '.omx', 'state', 'hud-state.json'), JSON.stringify({
        last_turn_at: new Date(now + 120_000).toISOString(),
        turn_count: 3,
      }, null, 2));
      await writeFile(join(wd, '.omx', 'state', 'team', 'dispatch-team', 'config.json'), JSON.stringify({
        name: 'dispatch-team',
      ...
      const tmuxLog = await readFile(tmuxLogPath, 'utf8');
      assert.match(tmuxLog, /send-keys -t %42 -l Team dispatch-team: worker panes stalled, no progress 3m(?:\d+s)?\./);
...
`;
  ...
  it('globally debounces Ralph continue steer across concurrent watcher instances', async () => {
    ...
    try {
      ...
      await Promise.all([
        waitForExit(first, WATCHER_PROCESS_EXIT_TIMEOUT_MS),
        waitForExit(second, WATCHER_PROCESS_EXIT_TIMEOUT_MS),
      ]);
  ...
  it('exits when the tracked parent pid is gone', async () => {
    ...
    try {
      ...
      await waitForExit(child, WATCHER_PROCESS_EXIT_TIMEOUT_MS);
      ...
      if (child && isPidAlive(child.pid)) {
        child.kill('SIGTERM');
        await waitForExit(child, WATCHER_PROCESS_EXIT_TIMEOUT_MS).catch(() => {});
      }
      await rm(wd, { recursive: true, force: true });
      await rm(tempHome, { recursive: true, force: true });
    }
  ...
  it('keeps ticking for active session-scoped Ralph after parent loss, then stops once Ralph is terminal', async () => {
    ...
    try {
      ...
      }, WATCHER_PROCESS_OBSERVATION_TIMEOUT_MS, 50);
  ...
  it('stays alive after parent exit while an active team still has live worker panes', async () => {
    ...
    try {
      ...
      await waitFor(async () => isPidAlive(child?.pid), WATCHER_PROCESS_OBSERVATION_TIMEOUT_MS, 50);
      await waitFor(async () => {
...
`);
      ...
      await waitFor(async () => !isPidAlive(helperPid), WATCHER_PROCESS_OBSERVATION_TIMEOUT_MS, 50);
    } finally {
      await rm(wd, { recursive: true, force: true });
    }
  });
