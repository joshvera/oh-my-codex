...
`,
  ...
  it("keeps SessionStart quiet when .omx/ is already ignored by repo-level gitignore", async () => {
    ...
    try {
      ...
      const gitignore = await readFile(join(cwd, ".gitignore"), "utf-8");
      assert.equal(gitignore, "node_modules/\n.omx/\n");
      const exclude = await readFile(join(cwd, ".git", "info", "exclude"), "utf-8").catch(() => "");
  ...
  it("does not expose submitted prompt text to keyword-detector hook plugins", async () => {
    ...
    const priorPluginTimeout = process.env.OMX_HOOK_PLUGIN_TIMEOUT_MS;
    try {
      process.env.OMX_HOOK_PLUGIN_TIMEOUT_MS = "60000";
      await mkdir(join(cwd, ".omx", "hooks"), { recursive: true });
      await writeFile(
...
`,
      ...
      if (typeof priorPluginTimeout === "string") {
        process.env.OMX_HOOK_PLUGIN_TIMEOUT_MS = priorPluginTimeout;
      } else {
